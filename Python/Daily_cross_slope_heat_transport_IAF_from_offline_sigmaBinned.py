...
if __name__ == '__main__':
	...
	dask.distributed.Client(n_workers = int(os.environ['PBS_NCPUS']),memory_limit = '380gib',local_directory = os.path.join(os.environ['PBS_JOBFS'], 'dask-worker-space'))
	...
	#src_HT='/g/data/x77/wf4500/ASC_project/Offline_tempflux/IAF/daily_heat_transport_z'
	...
	### Now that we have tempxflux_month loaded on the memory, lets bin it into density layers
	#Step 1, importing data to calculate desntity
	
	
	Temp = cc.querying.getvar(exp,'temp',session,ncfile='%daily%',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time),yt_ocean=lat_range) - 273.15
	Temp_u=Temp.interp(xt_ocean=xu_ocean).drop_vars('xt_ocean')
	Temp_v=Temp.interp(yt_ocean=yu_ocean).drop_vars('yt_ocean')
	
	Salt = cc.querying.getvar(exp,'salt',session,ncfile='%daily%',start_time=start_time,end_time=end_time).sel(time=slice(start_time,end_time),yt_ocean=lat_range)
	...
	st_ocean = cc.querying.getvar(exp,'st_ocean',session,n=1).load()
	time_monthly=Salt.time
	...
	pressure = xr.DataArray(p_from_z(depth_tile, lat_t),coords = [time_monthly,st_ocean, yt_ocean.sel(yt_ocean=lat_range), xt_ocean],dims = ['time','st_ocean','yt_ocean', 'xt_ocean'], name = 'pressure', attrs = {'units':'dbar'})
	
	ABS_Salt = SA_from_SP(Salt, pressure, lon_t, lat_t)
	Sigma0 = sigma0(ABS_Salt,Temp)
	Sigma0.name='potrho0'
	Sigma0.attrs= {'units':'kg/m3 -1000'}
	# interpoalting sigmas into the u,v grid
	Sigma0_u =Sigma0.interp(xt_ocean=xu_ocean)
	Sigma0_v =Sigma0.interp(yt_ocean=yu_ocean)
	
	
	
	### LOADING and binning temp_xflux #############################################
	#temp_xflux = temp_xflux.chunk(chunks={'st_ocean':25})
	temp_xflux.load()
	#freezing_point_heat_trans_zonal = freezing_point_heat_trans_zonal.chunk(chunks={'st_ocean':25})
	...
	#Sigma0_u =Sigma0_u.chunk(chunks={'st_ocean':25})
	Sigma0_u.load()
	...
	# binning temp_xflux into sigma layers
	temp_xflux_sigma = histogram(Sigma0_u, bins = [targetbins], dim = ['st_ocean'],weights = temp_xflux_month).rename({Sigma0_u.name + '_bin': 'potrho0'})
	
	
	del temp_xflux_month,pressure,Sigma0_u
	temp_xflux_month = temp_xflux_sigma
	...
	temp_yflux_sigma = histogram(Sigma0_v,bins = [targetbins], dim = ['st_ocean'],weights = temp_yflux_month).rename({Sigma0_v.name + '_bin': 'potrho0'})
	
	del temp_yflux_month,Sigma0_v
	temp_yflux_month = temp_yflux_sigma
	
	
	#transposing
	temp_xflux_month = temp_xflux_month.transpose('time','potrho0','yt_ocean','xu_ocean')
	temp_yflux_month = temp_yflux_month.transpose('time','potrho0','yu_ocean','xt_ocean')
	
	
	
	## Now we can extract the values along the contour!
	...
	cross_slope_heat_trans = np.zeros([ndays_month,108,n_bins])
	for lon_bin in range(n_bins):
	...
	zonal_heat_trans_west = np.zeros([ndays_month,108,n_bins])
	for lon_bin in range(n_bins):
		# west limit:
	...
	zonal_heat_trans_east = np.zeros([ndays_month,108,n_bins])
	for lon_bin in range(n_bins):
		# east limit:
		# reset these to False:
		on_x_grid = False
	...
	save_dir  = '/g/data/v45/wf4500/ASC_project_files/Cross_slope_heat_transport/OM2_IAF/daily_rho/Ant_cross_slope_heat_terms_offline_'+str(isobath_depth)+'m_'+ start_time+'.nc'    
	
	
	data_ZC=xr.DataArray((zonal_convergence),name="zonal_convergence",dims=["time","potrho0","lon_bin_midpoints"])
	...
	data_to_saveuv['potrho0']=np.array(heat_transp_across_contour.potrho0)
