# # Compute daily cross-slope-heat transport contour and save, use paralel looping for more efficient computation. Already discounts the freezing temperature
...
if __name__ == '__main__':
	...
	ndays_month=int(monthdays[int(int(sys.argv[1])-1)])
	...
	results = Parallel(n_jobs=-1)(delayed(HT_paralel)(monthdays,temp_yflux_with_mask_month,temp_xflux_with_mask_month,num_points,x_indices,y_indices,mask_y_transport_numbered,mask_x_transport_numbered,lon_along_contour,temp_xflux_month,temp_yflux_month,n) for n in range(ndays_month))
	...
	#fixing matrices shapes
	bheat_trans_across_contour=np.zeros([ndays_month,6002])
	bzonal_convergence=np.zeros([ndays_month,1428])
	bcross_slope_heat_trans=np.zeros([ndays_month,1428])
	for idays in range(ndays_month):
	...
	print('Opening coords for previous files')  
	
	#Importing pre-saved coords
	...
	ds = xr.Dataset({'zonal_convergence': zonal_convergence,'cross_slope_heat_trans':cross_slope_heat_trans,'lon_bin_midpoints':blon_bin_midpoints,'lat_bin_midpoints':blat_bin_midpoints,'bin_width':bbin_width,'bin_spacing':bbin_spacing,'heat_trans_across_contour':heat_trans_across_contour,'lon_along_contour':blon_along_contour})
